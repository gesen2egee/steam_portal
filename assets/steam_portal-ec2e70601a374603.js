(function () {
    ...
    function saveRemoteSyncConfig() {
      ...
      safeStorageSet(REMOTE_SYNC_CONFIG_KEY, JSON.stringify(remoteSyncConfig));
    }
    ...
    function renderRemoteSyncPanel() {
      ...
      el.settingsRemoteShareUrl.hidden = !remoteSyncConfig.rentryUrl;
    ...
    async function loadRemoteSync(value, automatic) {
      ...
      try {
        ...
        var loaded = await loadRemotePayload(manifest);
        ...
        saveRemoteSyncConfig();
        renderRemoteSyncFields();
        setRemoteSyncStatus("已讀取同步資料；清單 " + formatNumber(loaded.imported.lists.length) + " 個、固定河道 " + formatNumber(loaded.imported.rivers.length) + " 條");
        if (automatic) showPortalToast("已從跨裝置網址載入同步資料");
        else showPortalToast("同步資料讀取完成");
      } catch (error) {
        setRemoteSyncStatus(String(error && error.message ? error.message : "讀取同步資料失敗"), true);
        if (automatic) showPortalToast("跨裝置同步讀取失敗");
      } finally {
        remoteSyncBusy = false;
        updateRemoteSyncControls();
        renderRemoteSyncHistory();
    ...
    function maybeAutoLoadRemoteSync() {
      ...
      var rentryUrl = resolveRemoteSyncUrl(params.get("sync") || "");
      if (!rentryUrl) return;
