(function () {
    ...
    var STATE_EXPORT_VERSION = 7;
    var STATE_EXPORT_SOURCE = "steam";
    var SORT_VALUES = ["river", "river_reverse", "relevance", "updated", "release", "name", "price_asc", "price_desc", "review", "review_count"];
    var EXPLORE_SORT_VALUES = ["tag_weighted", "cluster", "random"];
    ...
    var NEXT_EXPLORE_PREFETCH_KEY = "steam_portal_next_explore_v1";
    var NEXT_EXPLORE_PREFETCH_SIZE = 100;
    ...
    var REMOTE_SYNC_CONFIG_KEY = "steam_portal_remote_sync_v1";
    var REMOTE_SYNC_FORMAT = "steam-portal-remote-sync";
    var REMOTE_SYNC_VERSION = 1;
    var REMOTE_SYNC_STATE_VERSION = 7;
    var REMOTE_SYNC_RENTRY_MAX_CHARS = 200000;
    var REMOTE_SYNC_CHUNK_CHARS = 145000;
    var REMOTE_SYNC_MAX_CHUNKS = 256;
    var REMOTE_SYNC_HISTORY_LIMIT = 50;
    var REMOTE_SYNC_MAX_ENCODED_CHARS = 20 * 1024 * 1024;
    var REMOTE_SYNC_REQUEST_TIMEOUT_MS = 30000;
    var REMOTE_SYNC_RENTRY_CREATE_DELAY_MS = 6500;
    var REMOTE_SYNC_RENTRY_HOSTS = new Set(["rentry.co", "rentry.org"]);
    ...
    var SYSTEM_LIST_DEFAULTS = {
      ...
      wishlist: { title: "願望", color: "pink", markRead: true, recommended: true },
      owned: { title: "已購買", color: "green", markRead: true, recommended: false }
    };
    ...
    var lastExportAt = 0;
    var remoteSyncConfig = { rentryUrl: "", editCode: "" };
    var remoteSyncManifest = null;
    var remoteSyncStatus = { message: "", isError: false };
    var remoteSyncBusy = false;
    var remoteSyncAutoLoadStarted = false;
    ...
    var mobilePreview = new URLSearchParams(window.location.search).get("mobile") === "1";
    ...
    var rivers = [];
    var exploreRiver = null;
    var pendingSearch = null;
    var nextExplorePrefetch = null;
    var nextExplorePrefetchTimer = 0;
    ...
    var settingsPage = "display";
    var listImportPreview = null;
    var wishlistImportPreview = null;
    var ownedImportPreview = null;
    var portalImportPreview = null;
    ...
    var loadBatchSize = 48;
    var riverVisibleCount = loadBatchSize;
    var riverLoadObserver = null;
    var mobileReadObserver = null;
    var mobileReadVisibleCards = new WeakMap();
    var mobileReadScrollSerial = 0;
    var mobileReadLastScrollY = null;
    var mobileReadLastScrollAt = 0;
    var mobileReadLayoutLockUntil = 0;
    ...
    var el = {
      ...
      mobileSort: document.getElementById("mobileSortSelect"),
      ...
      settingsRemoteSyncUrl: document.getElementById("settingsRemoteSyncUrl"),
      settingsRemoteShareUrl: document.getElementById("settingsRemoteShareUrl"),
      settingsRemotePublish: document.getElementById("settingsRemotePublish"),
      settingsRemoteLoad: document.getElementById("settingsRemoteLoad"),
      settingsRemoteCopy: document.getElementById("settingsRemoteCopy"),
      settingsRemoteHistory: document.getElementById("settingsRemoteHistory"),
      settingsRemoteRollback: document.getElementById("settingsRemoteRollback"),
      settingsRemoteStatus: document.getElementById("settingsRemoteStatus"),
      ...
      mobileSearchRiverList: document.getElementById("mobileSearchRiverList"),
      mobileSearchRiverToggle: document.getElementById("mobileSearchRiverToggle"),
      ...
      settingsListImportFile: document.getElementById("settingsListImportFile"),
      settingsListImportDrop: document.getElementById("settingsListImportDrop"),
      settingsListImportPreview: document.getElementById("settingsListImportPreview"),
      settingsApplyListImport: document.getElementById("settingsApplyListImport"),
      settingsOwnedMhtml: document.getElementById("settingsOwnedMhtml"),
      settingsOwnedMhtmlText: document.getElementById("settingsOwnedMhtmlText"),
      settingsOwnedMhtmlDrop: document.getElementById("settingsOwnedMhtmlDrop"),
      settingsPreviewOwnedMhtml: document.getElementById("settingsPreviewOwnedMhtml"),
      settingsApplyOwnedMhtml: document.getElementById("settingsApplyOwnedMhtml"),
      settingsOwnedImportPreview: document.getElementById("settingsOwnedImportPreview"),
      ...
      settingsWishlistFile: document.getElementById("settingsWishlistFile"),
      settingsWishlistDrop: document.getElementById("settingsWishlistDrop"),
      settingsImportWishlistJson: document.getElementById("settingsImportWishlistJson"),
      settingsApplyWishlistJson: document.getElementById("settingsApplyWishlistJson"),
      settingsWishlistImportPreview: document.getElementById("settingsWishlistImportPreview"),
      settingsStateImportText: document.getElementById("settingsStateImportText"),
      settingsStateImportDrop: document.getElementById("settingsStateImportDrop"),
      settingsApplyStateImport: document.getElementById("settingsApplyStateImport"),
      settingsStateImportPreview: document.getElementById("settingsStateImportPreview"),
      settingsManualSaleSyncButton: document.getElementById("settingsManualSaleSyncButton"),
      settingsSaleSyncSummary: document.getElementById("settingsSaleSyncSummary"),
      settingsTabs: document.getElementById("settingsTabs"),
      createListDialog: document.getElementById("createListDialog"),
      createListName: document.getElementById("createListName"),
      createListColor: document.getElementById("createListColor"),
      createListRead: document.getElementById("createListRead"),
      createListRecommended: document.getElementById("createListRecommended"),
      createListConfirm: document.getElementById("createListConfirm"),
      createListCancel: document.getElementById("createListCancel"),
      createListCancelButton: document.getElementById("createListCancelButton"),
    ...
    function initializePortalRivers(initialExploreGameId, renderAfter) {
      ensureSystemLists();
      var restoredPrefetch = restoreNextExplorePrefetch();
      prepareExploreRiver(initialExploreGameId, restoredPrefetch ? restoredPrefetch.ids : null);
      riverInitializationPending = false;
      var selectedRiver = restoreRiverStateFromUrl();
      favorites = new Set((listById("favorites") ? listById("favorites").ids : []).map(String));
    ...
    function normalizeGame(game) {
      ...
      normalized.clusterId = game.cluster_id === null || game.cluster_id === undefined ? "" : String(game.cluster_id).trim();
    ...
    function portalBuildId() {
      if (window.__STEAM_PORTAL_BUILD_ID__) return String(window.__STEAM_PORTAL_BUILD_ID__);
      var meta = document.querySelector('meta[name="steam-portal-build"]');
      return meta ? String(meta.getAttribute("content") || "") : "embedded";
    }
    ...
    function normalizeSort(value, fallback) {
      ...
      return SORT_VALUES.indexOf(key) >= 0 ? key : (fallback || "relevance");
    }

    function normalizeExploreSort(value) {
      var key = String(value || "");
      return EXPLORE_SORT_VALUES.indexOf(key) >= 0 ? key : "cluster";
    }

    function normalizeListSort(value) {
      return normalizeSort(value, "river");
    }
    ...
    function normalizePortalSettings(value) {
      ...
      return {
        ...
        exploreSort: normalizeExploreSort(source.exploreSort),
    ...
    function renderSettingsPanel() {
      ...
      document.querySelectorAll('input[name="settingsExploreSort"]').forEach(function (input) {
        input.checked = input.value === normalizeExploreSort(portalSettings.exploreSort);
      });
      ...
      if (el.createListColor) {
        el.createListColor.innerHTML = LIST_COLORS.map(function (color) {
    ...
    function renderSettingsPage() {
      var pages = ["display", "explore", "lists", "sync"];
      if (pages.indexOf(settingsPage) < 0) settingsPage = "display";
      if (el.settingsTabs) {
        Array.prototype.forEach.call(el.settingsTabs.querySelectorAll("[data-settings-page]"), function (button) {
          var active = button.getAttribute("data-settings-page") === settingsPage;
          button.classList.toggle("active", active);
          button.setAttribute("aria-selected", String(active));
        });
      }
      Array.prototype.forEach.call(document.querySelectorAll("[data-settings-page-content]"), function (page) {
        page.hidden = page.getAttribute("data-settings-page-content") !== settingsPage;
      });
    }

    function setSettingsPage(page) {
      if (["display", "explore", "lists", "sync"].indexOf(String(page)) < 0) return;
      settingsPage = String(page);
      renderSettingsPage();
    }
    ...
    function openCreateListDialog() {
      if (!el.createListDialog) return;
      el.createListName.value = "";
      el.createListColor.value = "blue";
      el.createListRead.checked = false;
      el.createListRecommended.checked = false;
      el.createListDialog.hidden = false;
      el.createListName.focus();
    }

    function closeCreateListDialog() {
      if (el.createListDialog) el.createListDialog.hidden = true;
    }

    function createList() {
      var title = String(el.createListName.value || "").trim() || "新清單";
      var now = Date.now();
      ...
      var list = {
        ...
        title: title,
        kind: "list",
        color: normalizeListColor(el.createListColor.value),
        markRead: !!el.createListRead.checked,
        recommended: !!el.createListRecommended.checked,
        listType: "custom",
        listSort: "river",
    ...
    function saveSelectedListSettings() {
      ...
      if (systemListDefaults(list.id)) title = listDefaultTitle(list.id);
      if (!title) {
    ...
    function parseTextListImport(rawText, list) {
      ...
      var existing = list ? new Set(list.ids) : new Set();
      return {
        ids: incoming,
        added: incoming.filter(function (appid) { return !existing.has(appid); }),
        duplicate: incoming.filter(function (appid) { return existing.has(appid); }),
        unmatched: unmatched
      };
    }

    function previewTextListImport() {
      var list = listById(settingsSelectedListId);
      if (!list) {
        el.settingsListImportStatus.textContent = "請先建立或選擇清單";
        if (el.settingsApplyListImport) el.settingsApplyListImport.disabled = true;
        return;
      }
      listImportPreview = parseTextListImport(el.settingsListImportText.value, list);
      el.settingsListImportPreview.textContent = "將新增 " + formatNumber(listImportPreview.added.length) + " 款；重複 " + formatNumber(listImportPreview.duplicate.length) + " 款；無法匹配 " + formatNumber(listImportPreview.unmatched.length) + " 行。套用策略：新增，不刪除。";
      el.settingsListImportStatus.textContent = listImportPreview.unmatched.length ? "請確認無法匹配的內容是否要修正後再預覽。" : "預覽完成，按「確認新增」才會寫入。";
      el.settingsApplyListImport.disabled = !listImportPreview.added.length;
    }

    function applyTextListImport() {
      var list = listById(settingsSelectedListId);
      if (!listImportPreview || !list) return;
      listImportPreview.added.forEach(function (appid) { list.ids.push(appid); });
      if (list.id === "favorites") {
        listImportPreview.added.forEach(function (appid) { favorites.add(appid); });
        syncFavoriteRiver();
      }
      list.updatedAt = Date.now();
      afterListMutation("已新增 " + formatNumber(listImportPreview.added.length) + " 款到「" + displayRiverTitle(list) + "」");
      el.settingsListImportText.value = "";
      el.settingsListImportPreview.textContent = "已套用：新增 " + formatNumber(listImportPreview.added.length) + " 款，重複 " + formatNumber(listImportPreview.duplicate.length) + " 款，忽略 " + formatNumber(listImportPreview.unmatched.length) + " 行。";
      el.settingsListImportStatus.textContent = "匯入完成";
      el.settingsApplyListImport.disabled = true;
      listImportPreview = null;
    }
    ...
    function readImportFile(file, onText, onError) {
      if (!file) return;
      var reader = new FileReader();
      reader.onload = function (event) {
        onText(String(event.target.result || ""));
      };
      reader.onerror = function () { if (onError) onError(); };
      reader.readAsText(file);
    }

    function previewOwnedMhtmlText(rawText) {
      var result = parseOwnedMhtml(rawText);
      ownedImportPreview = { ids: result.appids.slice(), steamId64: result.steamId64 };
      if (result.steamId64) {
        el.settingsSteamId64.value = result.steamId64;
        updateWishlistApiLink();
      }
      if (!result.appids.length) {
        el.settingsOwnedImportPreview.textContent = "找不到本站遊戲 App ID；請確認 Steam 遊戲頁已拉到底後再另存。";
        el.settingsApplyOwnedMhtml.disabled = true;
        setManualImportStatus("預覽失敗，尚未修改已購買清單", true);
        return;
      }
      var existing = listById("owned");
      var oldIds = existing ? new Set(existing.ids) : new Set();
      var duplicate = result.appids.filter(function (id) { return oldIds.has(id); }).length;
      var removed = existing ? existing.ids.filter(function (id) { return result.appids.indexOf(id) < 0; }).length : 0;
      el.settingsOwnedImportPreview.textContent = "將取代已購買清單：匯入 " + formatNumber(result.appids.length) + " 款；目前重複 " + formatNumber(duplicate) + " 款；會移除 " + formatNumber(removed) + " 款。" + (result.steamId64 ? " 已偵測 SteamID64。" : "");
      el.settingsApplyOwnedMhtml.disabled = false;
      setManualImportStatus("預覽完成，按「確認取代」才會寫入。", false);
    }

    function applyOwnedMhtmlImport() {
      if (!ownedImportPreview) return;
      var syncResult = syncImportedRiver("owned", ownedImportPreview.ids);
      prepareExploreRiver();
      renderSettingsPanel();
      render();
      el.settingsOwnedMhtmlText.value = "";
      el.settingsApplyOwnedMhtml.disabled = true;
      setManualImportStatus("已取代已購買清單：" + formatNumber(syncResult.total) + " 款；新增 " + formatNumber(syncResult.added) + " 款，移除 " + formatNumber(syncResult.removed) + " 款" + (ownedImportPreview.steamId64 ? "；已取得 SteamID64" : ""));
      ownedImportPreview = null;
    }

    function importOwnedMhtml(file) {
      if (!file) return;
      readImportFile(file, previewOwnedMhtmlText, function () { setManualImportStatus("無法讀取 MHTML 檔案", true); });
    }
    ...
    function previewWishlistJsonText() {
      var parsed;
      try { parsed = JSON.parse(String(el.settingsWishlistJson.value || "")); }
      catch (error) {
        wishlistImportPreview = null;
        el.settingsWishlistImportPreview.textContent = "內容不是有效的 JSON。";
        el.settingsApplyWishlistJson.disabled = true;
        setManualImportStatus("預覽失敗，尚未修改願望清單", true);
        return;
      }
      var ids = [];
      ...
      if (!ids.length) {
        wishlistImportPreview = null;
        el.settingsWishlistImportPreview.textContent = "JSON 中沒有匹配到本站遊戲 App ID。";
        el.settingsApplyWishlistJson.disabled = true;
        setManualImportStatus("預覽失敗，尚未修改願望清單", true);
        return;
      }
      var existing = listById("wishlist");
      var oldIds = existing ? new Set(existing.ids) : new Set();
      var duplicate = ids.filter(function (id) { return oldIds.has(id); }).length;
      var removed = existing ? existing.ids.filter(function (id) { return ids.indexOf(id) < 0; }).length : 0;
      wishlistImportPreview = { ids: ids.slice() };
      el.settingsWishlistImportPreview.textContent = "將取代願望清單：匯入 " + formatNumber(ids.length) + " 款；目前重複 " + formatNumber(duplicate) + " 款；會移除 " + formatNumber(removed) + " 款。";
      el.settingsApplyWishlistJson.disabled = false;
      setManualImportStatus("預覽完成，按「確認取代」才會寫入。", false);
    }

    function applyWishlistJsonImport() {
      if (!wishlistImportPreview) return;
      var syncResult = syncImportedRiver("wishlist", wishlistImportPreview.ids);
      prepareExploreRiver();
      renderSettingsPanel();
      render();
      el.settingsWishlistJson.value = "";
      el.settingsApplyWishlistJson.disabled = true;
      setManualImportStatus("已取代願望清單：" + formatNumber(syncResult.total) + " 款；新增 " + formatNumber(syncResult.added) + " 款，移除 " + formatNumber(syncResult.removed) + " 款");
      wishlistImportPreview = null;
    }
    ...
    function settingsVisibilityChanged(message) {
      ...
      riverVisibleCount = loadBatchSize;
    ...
    function captureExploreDefaults() {
      ...
      savePortalSettings();
      if (state.riverId === "explore") prepareExploreRiver(state.detailId);
      renderSettingsPanel();
    ...
    function normalizeRemoteSyncConfig(value) {
      var source = value && typeof value === "object" ? value : {};
      return {
        rentryUrl: normalizeRentryUrl(source.rentryUrl),
        editCode: String(source.editCode || "").trim().slice(0, 100)
      };
    }

    function loadRemoteSyncConfig() {
      return safeStorageGet(REMOTE_SYNC_CONFIG_KEY).then(function (raw) {
        if (!raw) return { rentryUrl: "", editCode: "" };
        try { return normalizeRemoteSyncConfig(JSON.parse(raw)); }
        catch (error) { return { rentryUrl: "", editCode: "" }; }
      });
    }

    function saveRemoteSyncConfig() {
      remoteSyncConfig = normalizeRemoteSyncConfig(remoteSyncConfig);
      return safeStorageSet(REMOTE_SYNC_CONFIG_KEY, JSON.stringify(remoteSyncConfig));
    }

    function setRemoteSyncStatus(message, isError) {
      remoteSyncStatus = { message: String(message || ""), isError: !!isError };
      if (!el || !el.settingsRemoteStatus) return;
      el.settingsRemoteStatus.textContent = remoteSyncStatus.message;
      el.settingsRemoteStatus.style.color = remoteSyncStatus.isError ? "var(--danger)" : "";
    }

    function updateRemoteSyncControls() {
      if (!el || !el.settingsRemotePublish) return;
      el.settingsRemotePublish.disabled = remoteSyncBusy;
      el.settingsRemoteLoad.disabled = remoteSyncBusy;
      el.settingsRemoteRollback.disabled = remoteSyncBusy || !remoteSyncManifest || !remoteSyncManifest.history.length;
      el.settingsRemoteCopy.disabled = remoteSyncBusy || !remoteSyncConfig.rentryUrl;
      el.settingsRemotePublish.textContent = remoteSyncBusy ? "同步處理中…" : "建立／更新同步";
    }

    function buildRemoteSyncLaunchUrl(rentryUrl) {
      var normalized = normalizeRentryUrl(rentryUrl);
      if (!normalized) return "";
      try {
        var url = new URL(window.location.href);
        var keepMobile = mobilePreview;
        url.search = "";
        url.hash = "";
        if (keepMobile) url.searchParams.set("mobile", "1");
        url.searchParams.set("sync", normalized);
        return url.toString();
      } catch (error) {
        return normalized;
      }
    ...
    function renderRemoteSyncHistory() {
      if (!el || !el.settingsRemoteHistory) return;
      var options = [];
      if (remoteSyncManifest) {
        options.push('<option value="current">目前版本（' + escapeHtml(remoteSyncManifest.stateUpdatedAt || "最新") + '）</option>');
        remoteSyncManifest.history.forEach(function (entry, index) {
          options.push('<option value="' + escapeHtml(String(index)) + '">第 ' + formatNumber(index + 1) + ' 個舊版本（' + escapeHtml(entry.stateUpdatedAt || "未知時間") + '）</option>');
        });
      }
      el.settingsRemoteHistory.innerHTML = options.length ? options.join("") : '<option value="">尚無歷史版本</option>';
      el.settingsRemoteHistory.disabled = !remoteSyncManifest || !remoteSyncManifest.history.length || remoteSyncBusy;
      if (!remoteSyncManifest || !remoteSyncManifest.history.length) el.settingsRemoteHistory.value = "";
    }

    function renderRemoteSyncPanel() {
      if (!el || !el.settingsRemoteSyncUrl) return;
      el.settingsRemoteSyncUrl.value = remoteSyncConfig.rentryUrl;
      el.settingsRemoteShareUrl.value = remoteSyncConfig.rentryUrl ? buildRemoteSyncLaunchUrl(remoteSyncConfig.rentryUrl) : "";
      el.settingsRemoteShareUrl.hidden = false;
      setRemoteSyncStatus(remoteSyncStatus.message, remoteSyncStatus.isError);
      renderRemoteSyncHistory();
      updateRemoteSyncControls();
    }
    ...
    function updateExportButton() {
      ...
      el.exportStateButton.title = due ? (lastExportAt ? "距上次匯出已超過 7 天" : "尚未匯出清單 JSON") : "下載清單與閱讀狀態 JSON";
    }
    ...
    function exportStatePayload() {
      var exportableRivers = rivers.map(serializeSavedRiver).filter(Boolean);
      return {
    ...
    function exportPortalState() {
      try {
        ...
        saveLastExportAt();
        updateExportButton();
        setBackupStatus("已匯出完整 Portal JSON；匯入時會與本機資料合併");
      } catch (error) {
    ...
    function normalizeRentryUrl(value) {
      ...
      try {
        ...
        if (url.protocol !== "https:" || !REMOTE_SYNC_RENTRY_HOSTS.has(host) || !/^[A-Za-z0-9_-]{2,100}$/.test(path)) return "";
    ...
    function resolveRemoteSyncUrl(value) {
      var text = String(value || "").trim();
      if (!text) return "";
      var direct = normalizeRentryUrl(text);
      if (direct) return direct;
      try {
        var url = new URL(text, window.location.href);
        var nested = url.searchParams.get("sync") || "";
        return normalizeRentryUrl(nested);
      } catch (error) {
        return "";
      }
    }

    function randomRemoteEditCode() {
      if (!window.crypto || !window.crypto.getRandomValues) throw new Error("此瀏覽器沒有安全亂數功能，無法建立同步網址");
      var alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789";
      var values = new Uint32Array(32);
      window.crypto.getRandomValues(values);
      return Array.prototype.map.call(values, function (value) { return alphabet[value % alphabet.length]; }).join("");
    }

    function remoteRequest(url, options, label) {
      if (!window.fetch || !window.AbortController) return Promise.reject(new Error("此瀏覽器不支援跨裝置同步所需的網路功能"));
      var controller = new AbortController();
      var requestOptions = Object.assign({}, options || {});
      requestOptions.credentials = "omit";
      requestOptions.cache = "no-store";
      requestOptions.signal = controller.signal;
      var timer = window.setTimeout(function () { controller.abort(); }, REMOTE_SYNC_REQUEST_TIMEOUT_MS);
      return window.fetch(url, requestOptions).then(function (response) {
        return response.text().then(function (body) {
          if (!response.ok) {
            ...
            var suffix = retryAfter ? "；請約 " + retryAfter + " 秒後重試" : "";
            throw new Error(label + "失敗（HTTP " + response.status + "）" + suffix);
          }
          return body;
        });
      }).catch(function (error) {
        if (error && error.name === "AbortError") throw new Error(label + "逾時，請稍後重試");
        throw error;
      }).then(function (value) {
    ...
    function rentryApiRequest(rentryUrl, path, fields, label) {
      var origin = new URL(rentryUrl).origin;
      var form = new URLSearchParams();
      Object.keys(fields || {}).forEach(function (key) { form.set(key, String(fields[key])); });
      return remoteRequest(origin + "/api/" + path, { method: "POST", body: form }, label).then(function (raw) {
        var response;
        try { response = JSON.parse(raw); }
        catch (error) { throw new Error(label + "回傳格式無法辨識"); }
        var status = String(response.status || "200");
        if (status !== "200") {
          var retry = status === "429" ? "；請稍後再試" : "";
          throw new Error(label + "失敗（服務狀態 " + status + "）" + retry);
        }
        return response.content && typeof response.content === "object" ? response.content : response;
      });
    }

    function rentrySlug(rentryUrl) {
      var path = new URL(rentryUrl).pathname.replace(/^\/+|\/+$/g, "");
      return path.split("/").pop() || "";
    }

    function rentryCreate(rentryUrl, text, editCode, label) {
      var fields = { text: text };
      if (editCode) fields.edit_code = editCode;
      var requestLabel = label || "建立 Rentry 貼文";
      return rentryApiRequest(rentryUrl, "new", fields, requestLabel).then(function (response) {
        var origin = new URL(rentryUrl).origin;
        var createdUrl = normalizeRentryUrl(response.url || response.url_short || "");
        if (!createdUrl && response.url) {
          try { createdUrl = normalizeRentryUrl(new URL(response.url, origin).toString()); } catch (error) {}
        }
        if (!createdUrl) throw new Error(requestLabel + "沒有回傳有效網址");
        return { rentryUrl: createdUrl, editCode: String(response.edit_code || editCode).trim().slice(0, 100) };
      });
    }

    function rentryEdit(rentryUrl, editCode, text) {
      if (!editCode) return Promise.reject(new Error("找不到 Rentry 編輯碼，無法更新主索引"));
      return rentryApiRequest(rentryUrl, "edit/" + encodeURIComponent(rentrySlug(rentryUrl)), { edit_code: editCode, text: text }, "更新 Rentry 主索引");
    }

    function remotePasteCandidates(html) {
      var parsed = new DOMParser().parseFromString(String(html || ""), "text/html");
      var nodes = Array.prototype.slice.call(parsed.querySelectorAll("pre, code, article"));
      var values = [];
      function add(value) {
        var text = String(value || "").trim();
        if (text && values.indexOf(text) < 0) values.push(text);
      }
      nodes.forEach(function (node) { add(node.textContent); });
      if (parsed.body) add(parsed.body.textContent);
      return values;
    }

    function stripRemoteCodeFence(value) {
      var text = String(value || "").trim();
    ...
    function normalizeRemoteVersionEntry(value) {
      if (!value || typeof value !== "object" || !Array.isArray(value.chunks) || !value.chunks.length || value.chunks.length > REMOTE_SYNC_MAX_CHUNKS) return null;
      var chunks = value.chunks.map(normalizeRentryUrl);
      if (chunks.some(function (url) { return !url; })) return null;
      var sha256 = String(value.sha256 || "").toLowerCase();
      if (!/^[0-9a-f]{64}$/.test(sha256)) return null;
      var chunkCount = Number(value.chunkCount || chunks.length);
      if (!Number.isInteger(chunkCount) || chunkCount !== chunks.length) return null;
      var compressedChars = Number(value.compressedChars || 0);
      if (!Number.isInteger(compressedChars) || compressedChars <= 0 || compressedChars > REMOTE_SYNC_MAX_ENCODED_CHARS) return null;
      return {
        stateUpdatedAt: String(value.stateUpdatedAt || value.updatedAt || ""),
    ...
    function normalizeRemoteManifest(value) {
      if (!value || typeof value !== "object" || value.format !== REMOTE_SYNC_FORMAT || Number(value.version) !== REMOTE_SYNC_VERSION || value.stateFormat !== STATE_EXPORT_FORMAT || Number(value.stateVersion) !== REMOTE_SYNC_STATE_VERSION || value.compression !== "gzip" || value.encoding !== "base64") {
        throw new Error("Rentry 主索引不是本入口的同步資料");
      }
      var current = normalizeRemoteVersionEntry(value);
      var editCode = String(value.editCode || "").trim();
      if (!current || !editCode || editCode.length > 100) throw new Error("Rentry 主索引缺少有效的分片或編輯碼");
      var history = (Array.isArray(value.history) ? value.history : []).map(normalizeRemoteVersionEntry).filter(Boolean).slice(0, REMOTE_SYNC_HISTORY_LIMIT);
      return {
        format: REMOTE_SYNC_FORMAT,
        version: REMOTE_SYNC_VERSION,
        stateFormat: STATE_EXPORT_FORMAT,
        stateVersion: REMOTE_SYNC_STATE_VERSION,
        compression: "gzip",
        encoding: "base64",
        ...
        compressedChars: current.compressedChars,
    ...
    function parseRemoteManifestHtml(html) {
      var candidates = remotePasteCandidates(html);
      for (var index = 0; index < candidates.length; index += 1) {
        try { return normalizeRemoteManifest(JSON.parse(stripRemoteCodeFence(candidates[index]))); }
        catch (error) {}
      }
      throw new Error("無法從 Rentry 主索引讀取同步設定");
    }

    function fetchRemoteManifest(rentryUrl) {
      return remoteRequest(rentryUrl, {}, "讀取 Rentry 主索引").then(parseRemoteManifestHtml);
    }
    ...
    function bytesToBase64(bytes) {
      ...
      for (var offset = 0; offset < bytes.length; offset += chunkSize) {
        binary += String.fromCharCode.apply(null, Array.prototype.slice.call(bytes, offset, Math.min(bytes.length, offset + chunkSize)));
      }
    ...
    function base64ToBytes(value) {
      var binary;
      try { binary = window.atob(String(value || "").replace(/\s+/g, "")); }
      catch (error) { throw new Error("同步資料的 Base64 格式錯誤"); }
      var bytes = new Uint8Array(binary.length);
      for (var index = 0; index < binary.length; index += 1) bytes[index] = binary.charCodeAt(index);
      return bytes;
    }

    function gzipTextToBase64(text) {
      if (!window.TextEncoder || !window.CompressionStream || !window.Response || !window.Blob) return Promise.reject(new Error("此瀏覽器不支援 GZIP 壓縮同步"));
      var bytes = new TextEncoder().encode(String(text || ""));
      var stream = new Blob([bytes]).stream().pipeThrough(new CompressionStream("gzip"));
      return new Response(stream).arrayBuffer().then(function (buffer) { return bytesToBase64(new Uint8Array(buffer)); });
    }

    function gunzipBase64ToText(value) {
      if (!window.DecompressionStream || !window.Response || !window.Blob) return Promise.reject(new Error("此瀏覽器不支援 GZIP 解壓縮同步"));
    ...
    function sha256Hex(value) {
      if (!window.crypto || !window.crypto.subtle || !window.TextEncoder) return Promise.reject(new Error("此瀏覽器不支援同步資料校驗"));
      return window.crypto.subtle.digest("SHA-256", new TextEncoder().encode(String(value || ""))).then(function (buffer) {
        return Array.prototype.map.call(new Uint8Array(buffer), function (byte) { return byte.toString(16).padStart(2, "0"); }).join("");
      });
    }

    function extractRemoteChunk(html) {
      var candidates = remotePasteCandidates(html);
      var best = "";
      candidates.forEach(function (candidate) {
        var text = stripRemoteCodeFence(candidate);
        if (!/^[A-Za-z0-9+/=_\r\n]+$/.test(text)) return;
        var compact = text.replace(/\s+/g, "");
        if (compact.length > best.length) best = compact;
      });
      if (!best) throw new Error("無法從 Rentry 分片讀取 Base64 內容");
      return best;
    }

    function createRentryChunk(content) {
      var text = "```text\n" + String(content || "") + "\n```";
      return rentryCreate("https://rentry.co/new", text, "", "建立 Rentry 分片").then(function (result) {
        return result.rentryUrl;
      });
    }

    function waitForRemoteSyncRateLimit() {
      return new Promise(function (resolve) {
        window.setTimeout(resolve, REMOTE_SYNC_RENTRY_CREATE_DELAY_MS);
      });
    }

    function remoteVersionRecord(manifest) {
      return {
        stateUpdatedAt: manifest.stateUpdatedAt,
        publishedAt: manifest.publishedAt,
        chunks: manifest.chunks.slice(),
        chunkCount: manifest.chunkCount,
        compressedChars: manifest.compressedChars,
        sha256: manifest.sha256
      };
    }

    function remoteVersionKey(entry) {
      return entry.sha256 + "|" + entry.chunks.join("|");
    }

    function uniqueRemoteHistory(entries) {
      var seen = new Set();
      return entries.filter(function (entry) {
        var key = remoteVersionKey(entry);
        if (seen.has(key)) return false;
        seen.add(key);
        return true;
      }).slice(0, REMOTE_SYNC_HISTORY_LIMIT);
    }

    function buildRemoteManifest(payload, encoded, sha256, chunks, editCode, history, stateUpdatedAt) {
      return {
        format: REMOTE_SYNC_FORMAT,
        version: REMOTE_SYNC_VERSION,
        stateFormat: STATE_EXPORT_FORMAT,
        ...
        compression: "gzip",
        encoding: "base64",
        stateUpdatedAt: String(stateUpdatedAt || payload.exportedAt || new Date().toISOString()),
        ...
        history: uniqueRemoteHistory(history || [])
      };
    }

    function buildRemoteManifestFromVersion(version, editCode, history) {
      return {
        format: REMOTE_SYNC_FORMAT,
        version: REMOTE_SYNC_VERSION,
        stateFormat: STATE_EXPORT_FORMAT,
        ...
        compression: "gzip",
        encoding: "base64",
        stateUpdatedAt: version.stateUpdatedAt,
        publishedAt: new Date().toISOString(),
        chunks: version.chunks.slice(),
        chunkCount: version.chunkCount,
        compressedChars: version.compressedChars,
    ...
    function remoteManifestMarkdown(manifest) {
      var markdown = "```json\n" + JSON.stringify(manifest, null, 2) + "\n```";
      if (markdown.length > REMOTE_SYNC_RENTRY_MAX_CHARS) throw new Error("同步歷史太多，主索引超過 Rentry 文字上限");
      return markdown;
    }

    async function loadRemotePayload(version) {
      var chunks = [];
      var encodedLength = 0;
      for (var index = 0; index < version.chunks.length; index += 1) {
        setRemoteSyncStatus("讀取分片 " + formatNumber(index + 1) + " / " + formatNumber(version.chunks.length) + "…");
        var html = await remoteRequest(version.chunks[index], {}, "讀取 Rentry 分片");
        var chunk = extractRemoteChunk(html);
        encodedLength += chunk.length;
        if (chunk.length > REMOTE_SYNC_RENTRY_MAX_CHARS || encodedLength > REMOTE_SYNC_MAX_ENCODED_CHARS) throw new Error("同步分片大小超過安全上限");
        chunks.push(chunk);
      }
      var encoded = chunks.join("");
      if (encoded.length !== version.compressedChars) throw new Error("同步分片長度不一致，可能有資料遺失");
      var actualHash = await sha256Hex(encoded);
      if (actualHash !== version.sha256) throw new Error("同步資料校驗失敗，沒有套用不完整資料");
      var json = await gunzipBase64ToText(encoded);
      return { json: json, imported: parseStateImport(json) };
    }

    async function loadRemotePayloadWithHistory(manifest) {
      var versions = [{ version: manifest, historyIndex: -1 }].concat(manifest.history.map(function (entry, index) {
        return { version: entry, historyIndex: index };
      }));
      var lastError = null;
      for (var index = 0; index < versions.length; index += 1) {
        var candidate = versions[index];
        if (candidate.historyIndex >= 0) {
          setRemoteSyncStatus("目前版本無法驗證，嘗試第 " + formatNumber(candidate.historyIndex + 1) + " 個歷史版本…");
        }
        try {
          var loaded = await loadRemotePayload(candidate.version);
          return { version: candidate.version, historyIndex: candidate.historyIndex, loaded: loaded };
        } catch (error) {
          lastError = error;
        }
      }
      throw lastError || new Error("沒有可用的同步版本");
    }

    function remoteSyncInputUrl() {
      var value = String(el.settingsRemoteSyncUrl.value || "").trim();
      if (!value) return "";
      var url = resolveRemoteSyncUrl(value);
      if (!url) throw new Error("請貼上有效的 Rentry 同步網址或跨裝置入口網址");
      return url;
    }

    function renderRemoteSyncFields() {
      if (!el || !el.settingsRemoteSyncUrl) return;
      el.settingsRemoteSyncUrl.value = remoteSyncConfig.rentryUrl;
      el.settingsRemoteShareUrl.value = remoteSyncConfig.rentryUrl ? buildRemoteSyncLaunchUrl(remoteSyncConfig.rentryUrl) : "";
      el.settingsRemoteShareUrl.hidden = false;
    ...
    async function publishRemoteSync() {
      if (remoteSyncBusy) return;
      remoteSyncBusy = true;
      updateRemoteSyncControls();
      try {
        var rentryUrl = remoteSyncInputUrl();
        var existingManifest = null;
        var editCode = "";
        if (rentryUrl) {
          setRemoteSyncStatus("讀取目前同步版本…");
          existingManifest = await fetchRemoteManifest(rentryUrl);
          editCode = existingManifest.editCode;
        } else {
          editCode = randomRemoteEditCode();
        }
        setRemoteSyncStatus("壓縮同步資料…");
        var payload = exportStatePayload();
        var serialized = JSON.stringify(payload);
        var encoded = await gzipTextToBase64(serialized);
        if (!encoded || encoded.length > REMOTE_SYNC_MAX_ENCODED_CHARS) throw new Error("壓縮後同步資料超過安全上限");
        var sha256 = await sha256Hex(encoded);
        var chunks = [];
        var chunkCount = Math.ceil(encoded.length / REMOTE_SYNC_CHUNK_CHARS);
        for (var index = 0; index < chunkCount; index += 1) {
          if (index > 0) await waitForRemoteSyncRateLimit();
          setRemoteSyncStatus("建立 Rentry 分片 " + formatNumber(index + 1) + " / " + formatNumber(chunkCount) + "…（每片間隔約 6.5 秒）");
          chunks.push(await createRentryChunk(encoded.slice(index * REMOTE_SYNC_CHUNK_CHARS, (index + 1) * REMOTE_SYNC_CHUNK_CHARS)));
        }
        var history = existingManifest ? [remoteVersionRecord(existingManifest)].concat(existingManifest.history) : [];
        var manifest = buildRemoteManifest(payload, encoded, sha256, chunks, editCode, history);
        var markdown = remoteManifestMarkdown(manifest);
        if (rentryUrl) {
          setRemoteSyncStatus("更新 Rentry 主索引…");
          await rentryEdit(rentryUrl, editCode, markdown);
        } else {
          setRemoteSyncStatus("建立 Rentry 主索引…");
          var created = await rentryCreate("https://rentry.co/new", markdown, editCode, "建立 Rentry 主索引");
          rentryUrl = created.rentryUrl;
          if (created.editCode && created.editCode !== editCode) {
            editCode = created.editCode;
            manifest.editCode = editCode;
            markdown = remoteManifestMarkdown(manifest);
            await rentryEdit(rentryUrl, editCode, markdown);
          }
        }
        remoteSyncConfig = { rentryUrl: rentryUrl, editCode: editCode };
        remoteSyncManifest = normalizeRemoteManifest(manifest);
        saveRemoteSyncConfig();
        renderRemoteSyncFields();
        setRemoteSyncStatus("同步完成；已保存 " + formatNumber(chunks.length) + " 個分片，歷史版本 " + formatNumber(remoteSyncManifest.history.length) + " 個");
        showPortalToast("跨裝置同步已更新");
      } catch (error) {
        setRemoteSyncStatus(String(error && error.message ? error.message : "同步失敗；舊主索引未變更"), true);
      } finally {
    ...
    async function loadRemoteSync(value, automatic) {
      if (remoteSyncBusy) return;
      remoteSyncBusy = true;
      updateRemoteSyncControls();
      try {
        var rentryUrl = resolveRemoteSyncUrl(value || el.settingsRemoteSyncUrl.value);
        if (!rentryUrl) throw new Error("請貼上有效的 Rentry 同步網址或跨裝置入口網址");
        setRemoteSyncStatus("讀取 Rentry 主索引…");
        var manifest = await fetchRemoteManifest(rentryUrl);
        remoteSyncConfig = { rentryUrl: rentryUrl, editCode: manifest.editCode };
        await saveRemoteSyncConfig();
        var selected = await loadRemotePayloadWithHistory(manifest);
        applyImportedPortalState(selected.loaded.imported, true);
        remoteSyncManifest = manifest;
        renderRemoteSyncFields();
        if (selected.historyIndex >= 0) {
          setRemoteSyncStatus("目前版本讀取失敗，已改用第 " + formatNumber(selected.historyIndex + 1) + " 個歷史版本；本機資料已同步，Rentry 主索引未修改");
          showPortalToast("目前版本有問題，已改用較早同步版本");
        } else {
          var importedLists = selected.loaded.imported.savedRivers.filter(function (river) { return river.kind === "list"; }).length;
          var importedSearches = selected.loaded.imported.savedRivers.filter(function (river) { return river.kind === "search"; }).length;
          setRemoteSyncStatus("已讀取同步資料；清單 " + formatNumber(importedLists) + " 個、搜尋河道 " + formatNumber(importedSearches) + " 條");
          if (automatic) showPortalToast("已從瀏覽器記住的同步網址載入資料");
          else showPortalToast("同步資料讀取完成");
        }
      } catch (error) {
        var message = String(error && error.message ? error.message : "讀取同步資料失敗");
        setRemoteSyncStatus(message + "；已保留本機資料，本次不套用同步", true);
        if (automatic) showPortalToast("同步失敗，已保留本機資料");
      } finally {
        remoteSyncBusy = false;
        updateRemoteSyncControls();
        renderRemoteSyncHistory();
    ...
    async function rollbackRemoteSync() {
      if (remoteSyncBusy || !remoteSyncManifest || !remoteSyncManifest.history.length) return;
      var selectedIndex = Number(el.settingsRemoteHistory.value);
      if (!Number.isInteger(selectedIndex) || selectedIndex < 0 || selectedIndex >= remoteSyncManifest.history.length) return;
      remoteSyncBusy = true;
      updateRemoteSyncControls();
      try {
        var selected = remoteSyncManifest.history[selectedIndex];
        setRemoteSyncStatus("驗證要回復的歷史版本…");
        var loaded = await loadRemotePayload(selected);
        var history = uniqueRemoteHistory([remoteVersionRecord(remoteSyncManifest)].concat(remoteSyncManifest.history));
        var selectedKey = remoteVersionKey(selected);
        history = history.filter(function (entry) { return remoteVersionKey(entry) !== selectedKey; });
        var manifest = buildRemoteManifestFromVersion(selected, remoteSyncConfig.editCode, history);
        var markdown = remoteManifestMarkdown(manifest);
        setRemoteSyncStatus("回復 Rentry 主索引…");
        await rentryEdit(remoteSyncConfig.rentryUrl, remoteSyncConfig.editCode, markdown);
        applyImportedPortalState(loaded.imported, true);
        remoteSyncManifest = normalizeRemoteManifest(manifest);
        saveRemoteSyncConfig();
        renderRemoteSyncFields();
        setRemoteSyncStatus("已回復歷史版本；目前歷史版本 " + formatNumber(remoteSyncManifest.history.length) + " 個");
        showPortalToast("同步資料已回復");
      } catch (error) {
        setRemoteSyncStatus(String(error && error.message ? error.message : "回復失敗；目前同步資料未變更"), true);
      } finally {
        remoteSyncBusy = false;
        updateRemoteSyncControls();
        renderRemoteSyncHistory();
    ...
    function copyRemoteSyncUrl() {
      var value = String(el.settingsRemoteShareUrl.value || "").trim();
      if (!value) return;
      function copied() {
        setRemoteSyncStatus("已複製跨裝置入口網址；請妥善保管，網址包含編輯權限");
        showPortalToast("已複製跨裝置入口網址");
      }
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(value).then(copied).catch(function () { setRemoteSyncStatus("瀏覽器拒絕剪貼簿，請手動複製入口網址", true); });
        return;
      }
      ...
      input.select();
      var copiedByFallback = false;
      try { copiedByFallback = document.execCommand("copy"); } catch (error) {}
      input.remove();
      if (copiedByFallback) copied(); else setRemoteSyncStatus("請手動複製入口網址", true);
    }

    function maybeAutoLoadRemoteSync() {
      if (remoteSyncAutoLoadStarted) return;
      var params = new URLSearchParams(window.location.search);
      var rentryUrl = resolveRemoteSyncUrl(params.get("sync") || "") || remoteSyncConfig.rentryUrl;
      if (!rentryUrl) return;
      remoteSyncAutoLoadStarted = true;
      el.settingsRemoteSyncUrl.value = rentryUrl;
      setRemoteSyncStatus("正在自動同步…");
      window.setTimeout(function () { loadRemoteSync(rentryUrl, true); }, 0);
    }
    ...
    function exportVisibleGameList() {
      try {
        ...
        if (current) {
          ...
          visibleGames = lastResults.slice(0, riverVisibleCount);
        }
        var lines = [];
    ...
    function parseStateImport(rawText) {
      ...
      if (!payload || payload.format !== STATE_EXPORT_FORMAT || Number(payload.version) !== STATE_EXPORT_VERSION || payload.source !== STATE_EXPORT_SOURCE || !payload.state || typeof payload.state !== "object" || !Array.isArray(payload.state.savedRivers)) {
        throw new Error("不是本入口匯出的同步資料");
      }
      return {
        readGames: normalizeImportedIds(payload.state.readGames),
        savedRivers: payload.state.savedRivers.map(function (river) { return normalizeRiver(river); }).filter(function (river) { return river && (river.kind === "list" || river.kind === "search"); }),
    ...
    function applyImportedPortalState(imported, replaceLocal) {
      var importedRivers = imported.savedRivers.slice();
      if (replaceLocal) {
        rivers = importedRivers.map(function (river) { return normalizeRiver(river); }).filter(Boolean);
        ensureSystemLists();
        favorites = new Set((listById("favorites") ? listById("favorites").ids : []).map(String));
        readGames = new Set(imported.readGames);
      } else {
      ...
      topicCounts = countTopicValues();
      saveRivers();
      nextExplorePrefetch = null;
      safeStorageSet(NEXT_EXPLORE_PREFETCH_KEY, "");
      state = readUrlState();
      initializePortalRivers(state.riverId === "explore" ? state.detailId : "", false);
      render();
      return imported;
    }

    function previewPortalState(rawText) {
      try {
        portalImportPreview = parseStateImport(rawText);
        var listCount = portalImportPreview.savedRivers.filter(function (river) { return river.kind === "list"; }).length;
        var searchCount = portalImportPreview.savedRivers.filter(function (river) { return river.kind === "search"; }).length;
        el.settingsStateImportPreview.textContent = "將合併保存資料：" + formatNumber(listCount) + " 個清單、" + formatNumber(searchCount) + " 條搜尋河道、" + formatNumber(portalImportPreview.readGames.length) + " 筆閱讀紀錄。探索河道與預取快取不會被匯入。";
        el.settingsApplyStateImport.disabled = false;
        setBackupStatus("備份預覽完成，按「確認合併」才會修改本機資料");
      } catch (error) {
        portalImportPreview = null;
        el.settingsStateImportPreview.textContent = String(error.message || "匯入失敗");
        el.settingsApplyStateImport.disabled = true;
        setBackupStatus(String(error.message || "匯入失敗"), true);
      }
    }

    function applyPortalStatePreview() {
      if (!portalImportPreview) return;
      var imported = portalImportPreview;
      applyImportedPortalState(imported, false);
      el.settingsStateImportText.value = "";
      el.settingsApplyStateImport.disabled = true;
      portalImportPreview = null;
      setBackupStatus("已確認合併：保存資料已寫入本機；探索河道會重新建立");
    }

    function bindImportSource(dropZone, fileInput, textArea, preview, onError) {
      if (!dropZone || !fileInput) return;
      function loadFile(file) {
        if (!file) return;
        readImportFile(file, function (text) {
    ...
    function loadRivers() {
      return safeStorageGet("steam_portal_saved_rivers_v2").then(function (raw) {
        if (!raw) return [];
        try {
          var values = JSON.parse(raw);
          if (!Array.isArray(values)) return [];
          return values.map(normalizeRiver).filter(function (river) { return !!river && (river.kind === "list" || river.kind === "search"); });
        } catch (error) { return []; }
      });
    }

    function normalizeNextExplorePrefetch(value) {
      if (!value || typeof value !== "object" || Number(value.version) !== 1) return null;
      var ids = [];
      var seen = new Set();
      (Array.isArray(value.ids) ? value.ids : []).forEach(function (item) {
        var id = String(item || "");
        if (!/^\d+$/.test(id) || seen.has(id) || ids.length >= NEXT_EXPLORE_PREFETCH_SIZE) return;
        seen.add(id);
        ids.push(id);
      });
      ...
      return {
        ...
        ids: ids,
        items: Array.isArray(value.items) ? value.items.slice(0, NEXT_EXPLORE_PREFETCH_SIZE) : [],
        createdAt: Number(value.createdAt || 0)
      };
    }

    function loadNextExplorePrefetch() {
      return safeStorageGet(NEXT_EXPLORE_PREFETCH_KEY).then(function (raw) {
        if (!raw) return null;
        try { return normalizeNextExplorePrefetch(JSON.parse(raw)); }
        catch (error) { return null; }
      });
    }

    function nextExplorePrefetchKey() {
      var readIds = Array.from(readGames).map(String).sort();
      var recommended = listRivers().filter(function (river) { return !!river.recommended; }).map(function (river) {
        return [river.id, river.updatedAt, river.ids.length].join(":");
      }).sort();
      return simpleHash(JSON.stringify({
        buildId: portalBuildId(),
        exploreSort: portalSettings.exploreSort,
        exploreDefaults: portalSettings.exploreDefaults,
        tagBlacklist: portalSettings.tagBlacklist,
        adultEnabled: portalSettings.adultEnabled,
        readIds: readIds,
        recommended: recommended
      }));
    }

    function nextRiverPreviewItem(game) {
      return {
        appid: game.appid,
        cluster_id: game.clusterId,
        name: game.name,
        translated_name: game.translated_name,
        subtitle: game.subtitle,
        summary: game.summary,
        core_point_labels: game.corePointLabels.slice(),
        tags: game.tags.slice(),
        genres: game.genres.slice(),
        categories: game.categories.slice(),
        release_date: game.release_date,
        release_year: game.releaseYear,
        is_free: game.is_free,
        has_demo: game.hasDemo,
        price_label: game.price_label,
        price_value: game.priceValue,
        price_original: game.price_original,
        price_current: game.price_current,
        historical_low: game.historicalLow,
        developers: game.developers.slice(),
        publishers: game.publishers.slice(),
        authors: game.authors.slice(),
        review: game.review,
        chinese_languages: game.languages.slice(),
        header_image: game.header_image,
        updated_ns: game.updatedNs
      };
    }

    function scheduleNextExplorePrefetch() {
      if (nextExplorePrefetchTimer) window.clearTimeout(nextExplorePrefetchTimer);
      nextExplorePrefetchTimer = window.setTimeout(function () {
        nextExplorePrefetchTimer = 0;
        var filters = normalizeExploreDefaults(portalSettings.exploreDefaults);
        var context = buildFilterContext(filters);
        var eligibleIds = games.filter(function (game) { return gameMatchesFilters(game, filters, context); }).map(function (game) { return game.appid; });
        var mode = normalizeExploreSort(portalSettings.exploreSort);
        var heat = mode === "tag_weighted" ? exploreTagHeat() : null;
        var clusterIds = mode === "cluster" ? exploreClusterIds() : null;
        var ids = buildExploreBucketOrder(eligibleIds, mode, heat, clusterIds).slice(0, NEXT_EXPLORE_PREFETCH_SIZE);
        if (!ids.length) return;
        var record = {
          version: 1,
          buildId: portalBuildId(),
          key: nextExplorePrefetchKey(),
          ids: ids,
          items: ids.map(function (id) { return nextRiverPreviewItem(gameMap[id]); }),
          createdAt: Date.now()
        };
        nextExplorePrefetch = record;
        safeStorageSet(NEXT_EXPLORE_PREFETCH_KEY, JSON.stringify(record));
      }, 0);
    }

    function restoreNextExplorePrefetch() {
      if (!nextExplorePrefetch || nextExplorePrefetch.buildId !== portalBuildId() || nextExplorePrefetch.key !== nextExplorePrefetchKey()) return null;
      var cachedIds = nextExplorePrefetch.ids.filter(function (id) {
        return !!gameMap[id] && gameAllowedBySettings(gameMap[id]);
      });
      nextExplorePrefetch = null;
      safeStorageSet(NEXT_EXPLORE_PREFETCH_KEY, "");
      return cachedIds.length ? { ids: cachedIds } : null;
    }
    ...
    function normalizeRiver(river) {
      if (!river || !river.id || String(river.id) === "explore" || String(river.id) === "default") return null;
      var ids = [];
      var seenIds = new Set();
      ...
      var config = river.config && typeof river.config === "object" ? river.config : {};
      var listLike = river.kind !== "search" || !!systemDefaults;
      var incomingTitle = String(river.title || "").trim();
      var defaultTitle = systemDefaults ? systemDefaults.title : listLike ? "新清單" : "搜尋河道";
      var createdAt = Number(river.createdAt || 0);
      var updatedAt = Number(river.updatedAt || river.createdAt || 0);
      var result = {
        id: riverId,
        title: String(systemDefaults ? systemDefaults.title : incomingTitle || defaultTitle),
      ...
      if (listLike) {
        result.color = normalizeListColor(config.color || (systemDefaults && systemDefaults.color) || "blue");
        result.markRead = typeof config.markRead === "boolean" ? config.markRead : listDefaultRead(riverId);
        result.recommended = typeof config.recommended === "boolean" ? config.recommended : listDefaultRecommended(riverId);
        result.listType = systemDefaults ? riverId : "custom";
        result.listSort = normalizeListSort(config.sort);
        result.filters = normalizeRiverFilters({ sort: "river" });
        result.config = {
          listType: result.listType,
          color: result.color,
          markRead: result.markRead,
          recommended: result.recommended,
          sort: result.listSort
        };
      } else {
        var searchConfig = config.filters && typeof config.filters === "object" ? config.filters : {};
        var source = ["query", "tag", "author", "similar"].indexOf(String(config.source || "")) >= 0 ? String(config.source) : "query";
        result.filters = normalizeRiverFilters(searchConfig);
        result.config = {
          source: source,
          filters: result.filters,
          sourceAppId: String(config.sourceAppId || "")
        };
      }
      return result;
    }
    ...
    function normalizeRiverFilters(filters) {
      ...
      return {
        ...
        sort: normalizeSort(filters.sort, "relevance")
      };
    }

    function serializeSavedRiver(river) {
      if (!river || (river.kind !== "list" && river.kind !== "search") || !river.id || river.id === "explore" || river.id === "default") return null;
      var normalized = normalizeRiver(river);
      if (!normalized) return null;
      var result = {
        id: normalized.id,
        kind: normalized.kind,
        title: normalized.title,
        ids: normalized.ids.slice(),
        position: normalized.position,
        createdAt: normalized.createdAt,
        updatedAt: normalized.updatedAt,
        config: Object.assign({}, normalized.config)
      };
      if (normalized.kind === "search") result.config.filters = normalizeRiverFilters(normalized.config.filters);
      return result;
    }

    function saveRivers() {
      riverViewCache = { key: "", ids: null };
      var serialized = rivers.map(serializeSavedRiver).filter(Boolean);
      safeStorageSet("steam_portal_saved_rivers_v2", JSON.stringify(serialized));
      updateStorageNote();
    }
    ...
    function ensureSystemLists() {
      var changed = false;
      Object.keys(SYSTEM_LIST_DEFAULTS).forEach(function (id) {
        var existing = rivers.find(function (river) { return river.id === id; });
        if (existing) {
          if (existing.kind !== "list") { existing.kind = "list"; changed = true; }
          if (existing.title !== listDefaultTitle(id)) { existing.title = listDefaultTitle(id); changed = true; }
          if (!Array.isArray(existing.ids)) { existing.ids = []; changed = true; }
          existing.color = normalizeListColor(existing.color || listDefaultColor(id));
          existing.markRead = typeof existing.markRead === "boolean" ? existing.markRead : listDefaultRead(id);
          existing.recommended = typeof existing.recommended === "boolean" ? existing.recommended : listDefaultRecommended(id);
          existing.listType = id;
          existing.listSort = normalizeListSort(existing.listSort);
          existing.filters = normalizeRiverFilters({ sort: "river" });
          existing.config = {
            listType: id,
            color: existing.color,
            markRead: existing.markRead,
            recommended: existing.recommended,
            sort: existing.listSort
          };
          return;
        }
        var now = Date.now();
        rivers.push(normalizeRiver({
          id: id,
          kind: "list",
          title: listDefaultTitle(id),
          ids: [],
          position: null,
          createdAt: now,
          updatedAt: now,
          config: {
            listType: id,
            color: listDefaultColor(id),
            markRead: listDefaultRead(id),
            recommended: listDefaultRecommended(id),
            sort: "river"
          }
        }));
        changed = true;
      });
      rivers = rivers.filter(Boolean).sort(riverOrder);
    ...
    function syncFavoriteRiver() {
      ...
      if (!existing) {
        ensureSystemLists();
        existing = rivers.find(function (river) { return river.id === "favorites"; });
      }
      ...
      var oldPosition = existing.position;
      ...
      var changed = oldIds.join("\u0000") !== nextIds.join("\u0000") || oldPosition !== nextPosition;
      existing.ids = nextIds;
      existing.position = nextPosition;
      existing.kind = "list";
      existing.title = listDefaultTitle("favorites");
      ...
      existing.filters = normalizeRiverFilters({ sort: "river" });
      existing.config = { listType: "favorites", color: existing.color, markRead: existing.markRead, recommended: existing.recommended, sort: existing.listSort };
    ...
    function initializeRiverPosition() {
      ...
      var nextPosition = river.id === "explore" || requestedPosition < 0 ? 0 : requestedPosition;
      if (river.position === nextPosition) return;
      river.position = nextPosition;
      river.updatedAt = Date.now();
      if (river.id !== "explore") saveRivers();
    }

    function createExploreRiver(ids, firstId) {
      var orderedIds = (Array.isArray(ids) ? ids : []).map(String).filter(function (id, index, values) {
        return !!gameMap[id] && values.indexOf(id) === index;
      });
      var first = String(firstId || "");
      if (first && gameMap[first] && gameAllowedBySettings(gameMap[first])) {
        orderedIds = [first].concat(orderedIds.filter(function (id) { return id !== first; }));
      }
      return {
        id: "explore",
        title: "探索河道",
        kind: "explore",
        filters: normalizeExploreDefaults(portalSettings.exploreDefaults),
        ids: orderedIds,
        position: orderedIds.length ? 0 : null,
        createdAt: 0,
        updatedAt: Date.now()
      };
    }
    ...
    function exploreClusterIds() {
      var clusterIds = new Set();
      exploreReferenceIds().forEach(function (id) {
        var game = gameMap[id];
        if (!game || !gameAllowedBySettings(game) || !game.clusterId) return;
        clusterIds.add(game.clusterId);
      });
      return clusterIds;
    }
    ...
    function buildExploreBucketOrder(ids, mode, heat, clusterIds) {
      var source = shuffleIds(ids.slice());
      if (source.length < 2) return source;
      var target = Math.floor(source.length / 2);
      var priority = [];
      if (mode === "tag_weighted" && heat) {
        ...
        priority = weighted.slice(0, target).map(function (item) { return item.id; });
      } else if (mode === "cluster" && clusterIds && clusterIds.size) {
        priority = shuffleIds(source.filter(function (id) {
          var game = gameMap[id];
          return !!(game && game.clusterId && clusterIds.has(game.clusterId));
        })).slice(0, target);
      }
      if (!priority.length) priority = shuffleIds(source).slice(0, target);
    ...
    function prepareExploreRiver(firstId, prefetchedIds) {
      ...
      var first = String(firstId || "");
      var hasFirst = !!first && gameMap[first] && gameAllowedBySettings(gameMap[first]);
      if (hasFirst) {
      ...
      var heat = mode === "tag_weighted" ? exploreTagHeat() : null;
      var clusterIds = mode === "cluster" ? exploreClusterIds() : null;
      var unreadOrder = buildExploreBucketOrder(unread, mode, heat, clusterIds);
      var readOrder = buildExploreBucketOrder(read, mode, heat, clusterIds);
      var generatedIds = (hasFirst ? [first] : []).concat(unreadOrder, readOrder);
      var cachedIds = Array.isArray(prefetchedIds) ? prefetchedIds.filter(function (id) {
        return !!gameMap[String(id)] && gameAllowedBySettings(gameMap[String(id)]);
      }).map(String) : [];
      var seen = new Set(cachedIds);
      var orderedIds = cachedIds.concat(generatedIds.filter(function (id) { return !seen.has(id); }));
      exploreRiver = createExploreRiver(orderedIds, firstId);
      scheduleNextExplorePrefetch();
    }
    ...
    function isProtectedRiver(id) {
      return String(id) === "explore" || String(id) === "default" || !!systemListDefaults(id);
    }

    function activeRiver() {
      if (riverInitializationPending) return null;
      if (state.riverId === "explore") return exploreRiver;
    ...
    function compareGamesForSort(a, b, sort, query) {
      var diff = 0;
      if (sort === "relevance") diff = relevanceScore(b, query) - relevanceScore(a, query) || b.updatedNs - a.updatedNs;
      else if (sort === "updated") diff = b.updatedNs - a.updatedNs;
      else if (sort === "release") diff = (b.releaseYear || 0) - (a.releaseYear || 0);
      else if (sort === "name") diff = a.name.localeCompare(b.name, "zh-Hant");
      else if (sort === "price_asc") diff = compareNullable(priceValueFor(a), priceValueFor(b), false);
      else if (sort === "price_desc") diff = compareNullable(priceValueFor(a), priceValueFor(b), true);
      else if (sort === "review") diff = b.reviewPercent - a.reviewPercent || b.reviewCount - a.reviewCount;
      else if (sort === "review_count") diff = b.reviewCount - a.reviewCount || b.reviewPercent - a.reviewPercent;
      return diff || a.name.localeCompare(b.name, "zh-Hant") || a.appid.localeCompare(b.appid);
    }

    function sortRiverIds(river, ids, sort) {
      if (!isListRiver(river)) return ids;
      var mode = normalizeListSort(sort || river.listSort);
      if (mode === "river") return ids;
      if (mode === "river_reverse") return ids.slice().reverse();
      var query = normalizeText(state.q);
      var indexed = ids.map(function (id, index) { return { id: id, index: index, game: gameMap[id] }; }).filter(function (entry) { return !!entry.game; });
      indexed.sort(function (a, b) { return compareGamesForSort(a.game, b.game, mode, query) || a.index - b.index; });
      return indexed.map(function (entry) { return entry.id; });
    }
    ...
    function riverIdsForReadFilter(river, includeId) {
      ...
      return sortRiverIds(river, ids, river.listSort);
    }
    ...
    function riverIdsForCurrentView(river, includeId) {
      ...
      ids = sortRiverIds(river, ids, isListRiver(river) ? state.sort : "river");
    ...
    function hasSearchRiverCondition() {
      return !!(pendingSearch || state.q || state.topics.size || state.authors.size);
    }
    ...
    function pendingSearchRiver() {
      ...
      var source = pendingSearch && pendingSearch.source ? pendingSearch.source : (state.authors.size ? "author" : state.topics.size ? "tag" : "query");
      var sourceAppId = pendingSearch && pendingSearch.sourceAppId ? String(pendingSearch.sourceAppId) : "";
      var defaultRiver = exploreRiver;
      var sourceIds = pendingSearch && Array.isArray(pendingSearch.sourceIds) ? pendingSearch.sourceIds : (Array.isArray(pendingRiverSourceIds) ? pendingRiverSourceIds : (defaultRiver ? defaultRiver.ids : []));
      if (!pendingSearch && !state.q && !state.topics.size && !state.authors.size) return null;
      var ids = sourceIds.filter(function (id) {
        return !!gameMap[id] && gameMatchesFilters(gameMap[id], scopeFilters, scopeContext);
      });
      return {
        id: "pending-search",
        title: pendingSearch && pendingSearch.title ? pendingSearch.title : riverTitle(filters),
        kind: "search",
        config: { source: source, filters: filters, sourceAppId: sourceAppId },
        filters: filters,
        ids: ids,
        position: null,
        createdAt: 0,
    ...
    function ensureRiverForState(results, id) {
      ...
      if (current && currentIndex >= 0) {
        ...
        if (current.id !== "explore") saveRivers();
        return current;
      }
      var pending = pendingSearchRiver();
      if (!pending) return null;
      var filters = snapshotFilters();
      var ids = pending.ids.slice();
      var requestedId = String(id);
      if (ids.indexOf(requestedId) < 0 && gameMap[requestedId]) ids.push(requestedId);
      if (ids.length <= 1) return null;
      var now = Date.now();
      var river = normalizeRiver({
        id: "river-" + now + "-" + Math.floor(Math.random() * 1000000),
        title: pending.title || riverTitle(filters),
        kind: "search",
        filters: filters,
        ids: ids,
    ...
    function createSimilarGamesRiver(id) {
      var sourceGame = gameMap[String(id || "")];
      if (!sourceGame || !sourceGame.clusterId) return;
      var similarIds = games.filter(function (game) {
        return game.appid !== sourceGame.appid && game.clusterId === sourceGame.clusterId && gameAllowedBySettings(game);
      }).map(function (game) { return game.appid; });
      if (!similarIds.length) {
        showPortalToast("找不到其他同聚類遊戲");
        return;
      }
      shuffleIds(similarIds);
      pendingSearch = {
        source: "similar",
        sourceAppId: sourceGame.appid,
        title: "類似《" + displayName(sourceGame) + "》的遊戲",
        sourceIds: similarIds
      };
      pendingRiverSourceIds = similarIds.slice();
      state = restoreRiverFilters({ sort: "river" });
      state.riverId = "";
      state.detailId = "";
      riverVisibleCount = loadBatchSize;
      syncUrl(true);
      render();
      showPortalToast("已暫存類似遊戲結果；滑過結果後會保存搜尋河道");
    }

    function jumpToTop() {
      if (isMobileLayout()) lockMobileReadObserver(700);
    ...
    function centerRiverGame(id) {
      if (isMobileLayout()) lockMobileReadObserver(700);
      var selector = isMobileLayout() ? '[data-mobile-game-card="' + String(id) + '"]' : '[data-open-game="' + String(id) + '"]';
      var target = el.gameList.querySelector(selector);
      if (!target) return;
      var root = document.documentElement;
      var body = document.body;
      var previousRootBehavior = root.style.scrollBehavior;
      ...
      root.style.scrollBehavior = "auto";
      body.style.scrollBehavior = "auto";
      target.scrollIntoView({ block: isMobileLayout() ? "start" : "center", inline: "nearest", behavior: "auto" });
      root.style.scrollBehavior = previousRootBehavior;
      body.style.scrollBehavior = previousBodyBehavior;
    }

    function openRiver(id, stayInRiver) {
      var river = String(id) === "explore" ? exploreRiver : rivers.find(function (item) { return item.id === String(id); });
      if (!river) return;
      pendingSearch = null;
      pendingRiverSourceIds = null;
      var restored = restoreRiverFilters(river.filters);
      if (isListRiver(river)) restored.sort = normalizeListSort(river.listSort);
      ...
      if (stayInRiver && focusId) {
        ...
        riverVisibleCount = Math.max(loadBatchSize, Math.min(visibleIds.length, Math.ceil((focusIndex + 1) / loadBatchSize) * loadBatchSize));
      } else {
        riverVisibleCount = loadBatchSize;
      }
      syncUrl(false);
    ...
    function riverHasDynamicFilter(river) {
      return !!(river && river.id !== "explore" && (river.id === "favorites" || river.filters.price === "sale" || river.filters.saleOnly || river.filters.minDiscount || river.filters.favoritesOnly));
    }
    ...
    function reconcileDynamicRivers() {
      ...
      var current = activeRiver();
      if (state.detailId && current && current.id !== "explore" && riverPosition(current, state.detailId) < 0) {
        state.detailId = "";
        syncUrl(true);
      }
    }
    ...
    function readUrlState() {
      var params = new URLSearchParams(window.location.search);
      var route = String(params.get("river") || "explore");
      var riverId = "explore";
      if (route === "explore") riverId = "explore";
      else if (route.indexOf("list:") === 0 && route.slice(5)) riverId = route.slice(5);
      else if (route.indexOf("search:") === 0 && route.slice(7)) riverId = route.slice(7);
      return {
        q: "",
        topics: new Set(),
        authors: new Set(),
    ...
    function restoreRiverStateFromUrl() {
      var requestedRiverId = String(state.riverId || "explore");
      var requestedDetailId = String(state.detailId || "");
      var selectedRiver = requestedRiverId === "explore" ? exploreRiver : rivers.find(function (river) { return river.id === requestedRiverId; });
      if (!selectedRiver) {
        requestedRiverId = "explore";
        selectedRiver = exploreRiver;
      }
      var restoredState = restoreRiverFilters(selectedRiver && selectedRiver.filters ? selectedRiver.filters : portalSettings.exploreDefaults);
      restoredState.riverId = requestedRiverId;
      restoredState.detailId = requestedDetailId;
      if (selectedRiver && isListRiver(selectedRiver)) restoredState.sort = normalizeListSort(selectedRiver.listSort);
      state = restoredState;
    ...
    function syncUrl(replace) {
      try {
        ...
        var routeId = String(state.riverId || "explore");
        var routeRiver = routeId === "explore" ? exploreRiver : rivers.find(function (river) { return river.id === routeId; });
        var route = routeId === "explore" || !routeRiver ? "explore" : (routeRiver.kind === "list" ? "list:" : "search:") + routeRiver.id;
        params.set("river", route);
    ...
    function getResults() {
      ...
      result.sort(function (a, b) { return compareGamesForSort(a, b, normalizeSort(state.sort, "relevance"), query); });
      return result;
    }
    ...
    function similarGamesButtonHtml(game) {
      var available = !!(game && game.clusterId);
      var label = available ? "類似遊戲" : "無聚類資料";
      var title = available ? "建立類似遊戲河道" : "這款遊戲沒有聚類資料";
      return '<button type="button" class="button similar-link" data-similar-games="' + escapeHtml(game ? game.appid : "") + '"' + (available ? '' : ' disabled') + ' aria-label="' + escapeHtml(title) + '" title="' + escapeHtml(title) + '">' + label + '</button>';
    }

    function displayRiverTitle(river) {
      if (river && river.title) return river.title;
      if (!river) return "";
      if (river.id === "explore") return "探索河道";
    ...
    function renderRivers() {
      ...
      el.riverToggle.textContent = riversCollapsed ? "展開搜尋河道" : "收起搜尋河道";
      var listShortcuts = listRivers().slice().sort(riverOrder);
      var shortcutHtml = '<button type="button" class="river-shortcut explore' + (current && current.id === "explore" ? ' active' : '') + '" data-open-river="explore" title="探索河道（' + escapeHtml(exploreRiver ? riverProgress(exploreRiver) : "0 / 0") + '）" aria-label="開啟探索河道">探索河道</button>';
      shortcutHtml += listShortcuts.map(function (river) {
      ...
      var searchRivers = rivers.filter(function (river) { return river.kind === "search"; }).slice().sort(riverOrder);
      var searchHtml = searchRivers.map(function (river) {
        var active = current && current.id === river.id;
        var title = displayRiverTitle(river);
        return '<div class="river-item"><button type="button" class="river-main search-river-main' + (active ? ' active' : '') + '" data-open-river="' + escapeHtml(river.id) + '" aria-label="開啟' + escapeHtml(title) + '"><span class="river-copy"><span class="river-title">' + escapeHtml(title) + '</span><span class="river-progress" aria-hidden="true"><i style="width:' + riverProgressPercent(river) + '%"></i></span></span><span class="river-meta">' + escapeHtml(riverProgress(river)) + '</span></button><button type="button" class="river-delete" data-delete-river="' + escapeHtml(river.id) + '" aria-label="刪除' + escapeHtml(title) + '">×</button></div>';
      }).join("");
      if (!riversCollapsed && current && current.pending) {
        searchHtml = '<div class="river-pending"><strong>' + escapeHtml(displayRiverTitle(current)) + '</strong><span>尚未保存；滑過結果後會加入搜尋河道</span></div>' + searchHtml;
      }
      el.riverList.innerHTML = riversCollapsed ? "" : (searchHtml || '<div class="river-empty">尚未保存搜尋河道。輸入搜尋、標籤或作者後滑過結果即可保存。</div>');
      var displayCurrent = current || exploreRiver;
      el.mobileRiverTitle.textContent = displayCurrent ? displayRiverTitle(displayCurrent) : "探索河道";
      el.mobileRiverProgress.textContent = displayCurrent ? riverProgress(displayCurrent) : "0 / 0";
      var mobileShortcutHtml = '<button type="button" class="mobile-river-option' + (current && current.id === "explore" ? ' active' : '') + '" data-open-river="explore"><span>探索河道</span><em>' + escapeHtml(exploreRiver ? riverProgress(exploreRiver) : "0 / 0") + '</em></button>' + listShortcuts.map(function (river) {
        var active = current && current.id === river.id;
        var title = displayRiverTitle(river);
        return '<button type="button" class="mobile-river-option' + (active ? ' active' : '') + '" data-open-river="' + escapeHtml(river.id) + '"><span>' + escapeHtml(title) + '</span><em>' + escapeHtml(riverProgress(river)) + '</em></button>';
      }).join("");
      if (el.mobileRiverShortcuts) el.mobileRiverShortcuts.innerHTML = mobileShortcutHtml;
      else el.mobileRiverList.innerHTML = mobileShortcutHtml;
      if (el.mobileSearchRiverList) {
        el.mobileSearchRiverList.hidden = riversCollapsed;
        var mobileSearchHtml = searchRivers.map(function (river) {
          var active = current && current.id === river.id;
          return '<button type="button" class="mobile-river-option search-river-option' + (active ? ' active' : '') + '" data-open-river="' + escapeHtml(river.id) + '"><span>' + escapeHtml(displayRiverTitle(river)) + '</span><em>' + escapeHtml(riverProgress(river)) + '</em></button>';
        }).join("");
        if (!riversCollapsed && current && current.pending) {
          mobileSearchHtml = '<div class="river-pending"><strong>' + escapeHtml(displayRiverTitle(current)) + '</strong><span>尚未保存；滑過結果後會加入搜尋河道</span></div>' + mobileSearchHtml;
        }
        el.mobileSearchRiverList.innerHTML = mobileSearchHtml || '<div class="river-empty">尚未保存搜尋河道</div>';
      }
      if (el.mobileSearchRiverToggle) {
        el.mobileSearchRiverToggle.textContent = riversCollapsed ? "展開" : "收起";
        el.mobileSearchRiverToggle.setAttribute("aria-expanded", String(!riversCollapsed));
      }
    }
    ...
    function renderResults() {
      ...
      var current = activeRiver() || pendingSearchRiver();
      riverVisibleCount = Math.max(loadBatchSize, Math.min(riverVisibleCount, lastResults.length || loadBatchSize));
      var visible = lastResults.slice(0, riverVisibleCount);
      var shownStart = visible.length ? 1 : 0;
      var shownEnd = visible.length ? visible.length : 0;
      ...
      refreshMobileTitleMarquees();
      refreshMobileCoreMarquees();
      observeMobileReadCards();
      renderPagination();
      renderActiveFilters();
      renderRivers();
    }

    function refreshMobileTitleMarquees() {
      if (!isMobileLayout()) return;
      var titleNodes = el.gameList.querySelectorAll(".mobile-game-title");
      Array.prototype.forEach.call(titleNodes, function (titleNode) {
        var textNode = titleNode.querySelector(".mobile-game-title-text");
        if (!textNode) return;
        titleNode.classList.remove("is-overflow");
        textNode.style.removeProperty("--mobile-title-shift");
        textNode.style.removeProperty("--mobile-title-duration");
        var availableWidth = titleNode.clientWidth;
        var textWidth = Math.ceil(textNode.getBoundingClientRect().width);
        if (availableWidth > 0 && textWidth > availableWidth + 2) {
          textNode.style.setProperty("--mobile-title-shift", String(availableWidth - textWidth) + "px");
          textNode.style.setProperty("--mobile-title-duration", Math.max(6, Math.min(14, textWidth / 28)) + "s");
          titleNode.classList.add("is-overflow");
        }
      });
    }

    function refreshMobileCoreMarquees() {
      if (!isMobileLayout()) return;
      var listNodes = el.gameList.querySelectorAll(".mobile-summary-core-list");
      Array.prototype.forEach.call(listNodes, function (listNode) {
        var trackNode = listNode.querySelector(".mobile-summary-core-track");
        if (!trackNode) return;
        listNode.classList.remove("is-overflow");
        trackNode.style.removeProperty("--mobile-core-shift");
        trackNode.style.removeProperty("--mobile-core-duration");
        var availableWidth = listNode.clientWidth;
        var contentWidth = Math.ceil(trackNode.getBoundingClientRect().width);
        if (availableWidth > 0 && contentWidth > availableWidth + 2) {
          trackNode.style.setProperty("--mobile-core-shift", String(availableWidth - contentWidth) + "px");
          trackNode.style.setProperty("--mobile-core-duration", Math.max(8, Math.min(18, contentWidth / 26)) + "s");
          listNode.classList.add("is-overflow");
        }
      });
    }

    function markMobileCardRead(id) {
      var appid = String(id || "");
      var game = gameMap[appid];
      if (!game || !gameAllowedBySettings(game)) return;
      var river = activeRiver();
      if (river && riverPosition(river, appid) >= 0) updateRiverPosition(appid);
      var shouldCreateSearchRiver = !river && hasSearchRiverCondition();
      if (!isGameRead(appid)) markGameRead(appid);
      if (!shouldCreateSearchRiver) {
        renderRivers();
        return;
      }
      var newRiver = ensureRiverForState(lastResults, appid);
      if (!newRiver) {
        renderRivers();
        return;
      }
      syncUrl(true);
      renderResultsPreservingMobileCard(appid);
    }

    function lockMobileReadObserver(delay) {
      mobileReadLayoutLockUntil = Math.max(mobileReadLayoutLockUntil, Date.now() + Number(delay || 0));
    }

    function noteMobileScroll() {
      if (!isMobileLayout()) return;
      var nextY = window.scrollY;
      if (mobileReadLastScrollY === null) {
        mobileReadLastScrollY = nextY;
        return;
      }
      if (Math.abs(nextY - mobileReadLastScrollY) < 0.5) return;
      mobileReadLastScrollY = nextY;
      if (Date.now() < mobileReadLayoutLockUntil) return;
      mobileReadScrollSerial += 1;
      mobileReadLastScrollAt = Date.now();
    }

    function observeMobileReadCards() {
      if (mobileReadObserver) mobileReadObserver.disconnect();
      mobileReadObserver = null;
      mobileReadVisibleCards = new WeakMap();
      if (!isMobileLayout() || typeof window.IntersectionObserver !== "function") return;
      mobileReadLastScrollY = window.scrollY;
      var cards = el.gameList.querySelectorAll("[data-mobile-game-card]");
      if (!cards.length) return;
      mobileReadObserver = new IntersectionObserver(function (entries) {
        var viewportBottom = window.innerHeight || document.documentElement.clientHeight || 0;
        Array.prototype.forEach.call(entries, function (entry) {
          var card = entry.target;
          if (entry.isIntersecting) {
            mobileReadVisibleCards.set(card, { scrollSerial: mobileReadScrollSerial, enteredAt: Date.now() });
            return;
          }
          var visibleState = mobileReadVisibleCards.get(card);
          if (!visibleState) return;
          if (Date.now() < mobileReadLayoutLockUntil) return;
          if (visibleState.scrollSerial === mobileReadScrollSerial || mobileReadLastScrollAt < visibleState.enteredAt) return;
          var rect = card.getBoundingClientRect();
          if (rect.bottom <= 0 || rect.top >= viewportBottom) markMobileCardRead(card.getAttribute("data-mobile-game-card"));
        });
      }, { threshold: 0 });
      Array.prototype.forEach.call(cards, function (card) { mobileReadObserver.observe(card); });
    }
    ...
    function mobileIconSvg(kind, active) {
      ...
      if (kind === "similar") return '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="6" cy="6" r="2.5"></circle><circle cx="18" cy="6" r="2.5"></circle><circle cx="12" cy="18" r="2.5"></circle><path d="m8.2 7.1 2.5 8M15.8 7.1l-2.5 8M8.7 6h6.6"></path></svg>';
      if (kind === "essay") return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 8h14l-7 8z"></path></svg>';
    ...
    function mobileSimilarActionHtml(game, extraClass) {
      var available = !!(game && game.clusterId);
      var classes = "mobile-icon-action" + (extraClass ? " " + extraClass : "") + " similar";
      var title = available ? "建立類似遊戲河道" : "這款遊戲沒有聚類資料";
      return '<button type="button" class="' + classes + '" data-similar-games="' + escapeHtml(game.appid) + '"' + (available ? '' : ' disabled') + ' aria-label="' + escapeHtml(title) + '" title="' + escapeHtml(title) + '">' + mobileIconSvg("similar") + '</button>';
    }
    ...
    function mobileEssayFooterActionsHtml(game) {
      ...
      var steamUrl = 'https://store.steampowered.com/app/' + encodeURIComponent(game.appid) + '/';
      var steamDbUrl = 'https://steamdb.info/app/' + encodeURIComponent(game.appid) + '/';
      return '<a class="mobile-icon-action mobile-essay-action steam" href="' + steamUrl + '" target="_blank" rel="noopener" aria-label="前往 Steam" title="前往 Steam">' + mobileIconSvg("steam") + '</a>' +
        '<a class="mobile-icon-action mobile-essay-action steamdb" href="' + steamDbUrl + '" target="_blank" rel="noopener" aria-label="前往 SteamDB" title="前往 SteamDB">' + mobileIconSvg("database") + '</a>' +
        mobileSimilarActionHtml(game, "mobile-essay-action") +
        '<button type="button" class="mobile-icon-action mobile-essay-action favorite' + (favorite ? ' active' : '') + '" data-favorite-id="' + escapeHtml(game.appid) + '" aria-label="' + (favorite ? '取消收藏' : '加入收藏') + '" aria-pressed="' + String(favorite) + '" title="' + (favorite ? '取消收藏' : '加入收藏') + '">' + mobileIconSvg("star", favorite) + '</button>';
    }
    ...
    function mobilePriceHtml(game) {
      var info = livePriceInfo(game);
      var classes = ["mobile-fact-price"];
      if (info.isSale) classes.push("sale-price");
      if (isHistoricalLow(game)) classes.push("historical-low-price");
      return '<span class="' + classes.join(" ") + '">' + escapeHtml(info.current) + '</span>';
    }
    ...
    function mobileGameCardHtml(game) {
      ...
      var price = mobilePriceHtml(game);
      var language = languageText(game.languages);
      var expanded = expandedMobileGameIds.has(game.appid);
      var summaryCore = coreLabels ? '<div class="mobile-summary-core" aria-label="遊戲三大重點"><div class="mobile-summary-core-list"><div class="mobile-summary-core-track">' + coreLabels + '</div></div></div>' : '';
      return '<article class="mobile-game-card" data-mobile-game-card="' + escapeHtml(game.appid) + '"><section class="mobile-game-visual">' + coverHtml + '<div class="mobile-game-shade"></div><div class="mobile-game-overlay"><div class="mobile-game-title-row"><h3 class="mobile-game-title" title="' + escapeHtml(title) + '"><span class="mobile-game-title-text">' + escapeHtml(title) + '</span></h3>' + mobileGameActionsHtml(game, "cover") + '</div>' + (genres ? '<div class="mobile-genre-strip">' + genres + '</div>' : '') + '<div class="mobile-game-facts"><span class="mobile-fact-language ' + (language === "無中文" ? "language-none" : "") + '">' + escapeHtml(language) + '</span><span class="mobile-fact-primary"><span>' + review + '</span><span>' + price + '</span></span></div></div></section><section class="mobile-summary-row"><p>' + escapeHtml(game.summary || '暫無摘要。') + '</p><button type="button" class="mobile-essay-toggle" data-mobile-essay-toggle="' + escapeHtml(game.appid) + '" aria-expanded="' + String(expanded) + '" aria-label="' + (expanded ? '收起玩家心得' : '展開玩家心得') + '" title="' + (expanded ? '收起玩家心得' : '展開玩家心得') + '">' + mobileIconSvg("essay") + '</button></section>' + summaryCore + mobileEssayRegionHtml(game, title) + '</article>';
    }
    ...
    function loadMoreRiverResults() {
      if (!(activeRiver() || pendingSearchRiver()) || riverVisibleCount >= lastResults.length) return;
      riverVisibleCount = Math.min(lastResults.length, riverVisibleCount + loadBatchSize);
      renderResults();
    }

    function observeRiverLoadMore() {
      if (riverLoadObserver) riverLoadObserver.disconnect();
      if (!(activeRiver() || pendingSearchRiver()) || riverVisibleCount >= lastResults.length || typeof window.IntersectionObserver !== "function") return;
    ...
    function renderDetail(game) {
      ...
      var similarAction = similarGamesButtonHtml(game);
      var screenshotUrls = game.screenshots.slice(0, 12).map(safeUrl).filter(Boolean);
      ...
      var sidePrevious = '<button type="button" class="detail-side-nav side-prev"' + (previous ? ' data-open-game="' + escapeHtml(previous.appid) + '"' : ' disabled') + ' aria-label="上一款遊戲">上一款</button>';
      ...
      var sideNext = '<button type="button" class="detail-side-nav side-next"' + (next ? ' data-open-game="' + escapeHtml(next.appid) + '"' : ' disabled') + ' aria-label="下一款遊戲">下一款</button>';
      var sideFavoriteNext = '<button type="button" class="detail-side-favorite-next" data-favorite-next="' + escapeHtml(game.appid) + '"' + (next ? '' : ' disabled') + ' aria-label="收藏並下一款">收藏並下一款</button>';
    ...
    function updateManualSaleSyncButton() {
      if (!el) return;
      var label = saleSync.busy ? "更新中…" : "更新價格資料";
      if (el.manualSaleSyncButton) {
        el.manualSaleSyncButton.disabled = !!saleSync.busy;
        el.manualSaleSyncButton.textContent = label;
      }
      if (el.settingsManualSaleSyncButton) {
        el.settingsManualSaleSyncButton.disabled = !!saleSync.busy;
        el.settingsManualSaleSyncButton.textContent = label;
      }
      if (el.settingsSaleSyncSummary && el.saleSyncAge) el.settingsSaleSyncSummary.textContent = el.saleSyncAge.textContent;
    }
    ...
    function closeDetail() {
      ...
      if (currentRiver && leavingId) {
        ...
        if (currentIndex >= 0) {
          riverVisibleCount = Math.max(riverVisibleCount, Math.min(currentIds.length, Math.ceil((currentIndex + 1) / loadBatchSize) * loadBatchSize));
        }
      }
      state.detailId = "";
      syncUrl(false);
    ...
    function syncFormControls() {
      ...
      el.sort.value = normalizeSort(state.sort, "relevance");
      if (el.mobileSort) el.mobileSort.value = normalizeSort(state.sort, "relevance");
    ...
    function updateSortOptionLabel() {
      ...
      var current = activeRiver();
      var listActive = !!(current && isListRiver(current));
      riverOption.textContent = current && current.id === "explore" ? "探索河道順序" : listActive ? "加入順序" : "河道記錄順序";
      var reverseOption = el.sort.querySelector('option[value="river_reverse"]');
      if (reverseOption) {
        reverseOption.textContent = listActive ? "加入順序（反向）" : "河道順序（反向）";
        reverseOption.disabled = !listActive;
      }
      if (el.mobileSort) {
        var mobileRiverOption = el.mobileSort.querySelector('option[value="river"]');
        if (mobileRiverOption) mobileRiverOption.textContent = riverOption.textContent;
        var mobileReverseOption = el.mobileSort.querySelector('option[value="river_reverse"]');
        if (mobileReverseOption) {
          mobileReverseOption.textContent = listActive ? "加入順序（反向）" : "河道順序（反向）";
          mobileReverseOption.disabled = !listActive;
        }
      }
    }
    ...
    function rememberActiveRiverFilters() {
      ...
      current.config = { source: current.config && current.config.source || "query", filters: nextFilters, sourceAppId: current.config && current.config.sourceAppId || "" };
      current.updatedAt = Date.now();
      saveRivers();
    }
    ...
    function beginFilterChange(searchLike) {
      ...
      if (searchLike) pendingSearch = null;
      state.detailId = "";
      riverVisibleCount = loadBatchSize;
    }
    ...
    el.settingsTabs.addEventListener("click", function (event) {
      var button = event.target.closest("[data-settings-page]");
      if (button) setSettingsPage(button.getAttribute("data-settings-page"));
    });
    el.settingsBackdrop.addEventListener("change", function (event) {
      ...
      if (target.name === "settingsNameMode") {
        ...
        if (state.detailId) rerenderDetailAtCurrentScroll(); else render();
        renderSettingsPanel();
      } else if (target.name === "settingsExploreSort") {
        portalSettings.exploreSort = normalizeExploreSort(target.value);
        savePortalSettings();
        prepareExploreRiver(state.detailId);
        render();
        renderSettingsPanel();
    ...
    el.settingsCreateList.addEventListener("click", openCreateListDialog);
    el.createListConfirm.addEventListener("click", createList);
    el.createListCancel.addEventListener("click", closeCreateListDialog);
    el.createListCancelButton.addEventListener("click", closeCreateListDialog);
    ...
    el.settingsImportList.addEventListener("click", previewTextListImport);
    el.settingsApplyListImport.addEventListener("click", applyTextListImport);
    ...
    el.settingsPreviewOwnedMhtml.addEventListener("click", function () { previewOwnedMhtmlText(el.settingsOwnedMhtmlText.value); });
    el.settingsApplyOwnedMhtml.addEventListener("click", applyOwnedMhtmlImport);
    ...
    el.settingsImportWishlistJson.addEventListener("click", previewWishlistJsonText);
    el.settingsApplyWishlistJson.addEventListener("click", applyWishlistJsonImport);
    bindImportSource(el.settingsListImportDrop, el.settingsListImportFile, el.settingsListImportText, previewTextListImport, function () {
      el.settingsListImportStatus.textContent = "無法讀取文字清單檔案";
    });
    bindImportSource(el.settingsOwnedMhtmlDrop, el.settingsOwnedMhtml, el.settingsOwnedMhtmlText, previewOwnedMhtmlText, function () {
      setManualImportStatus("無法讀取 MHTML 檔案", true);
    });
    bindImportSource(el.settingsWishlistDrop, el.settingsWishlistFile, el.settingsWishlistJson, previewWishlistJsonText, function () {
      setManualImportStatus("無法讀取願望清單 JSON 檔案", true);
    });
    function changeSort(value) {
      var current = activeRiver();
      beginFilterChange(false);
      var nextSort = normalizeSort(value, "relevance");
      var listActive = !!(current && isListRiver(current));
      if (nextSort === "river_reverse" && !listActive) nextSort = "river";
      state.sort = nextSort;
      if (listActive) {
        current.listSort = normalizeListSort(nextSort);
        current.updatedAt = Date.now();
        saveRivers();
      } else if (state.sort === "river") {
        if (hasSearchFilters()) state.sort = "relevance";
        else state.riverId = "explore";
      }
      rememberActiveRiverFilters();
      syncUrl(true);
      render();
    }
    el.sort.addEventListener("change", function () { changeSort(el.sort.value); });
    if (el.mobileSort) el.mobileSort.addEventListener("change", function () { changeSort(el.mobileSort.value); });
    el.manualSaleSyncButton.addEventListener("click", runManualSaleSync);
    el.settingsManualSaleSyncButton.addEventListener("click", runManualSaleSync);
    el.settingsRemotePublish.addEventListener("click", publishRemoteSync);
    el.settingsRemoteLoad.addEventListener("click", function () { loadRemoteSync("", false); });
    el.settingsRemoteCopy.addEventListener("click", copyRemoteSyncUrl);
    el.settingsRemoteRollback.addEventListener("click", rollbackRemoteSync);
    ...
    el.importStateButton.addEventListener("click", function () {
      var rawText = String(el.settingsStateImportText.value || "").trim();
      if (rawText) previewPortalState(rawText); else el.importStateFile.click();
    });
    el.settingsApplyStateImport.addEventListener("click", applyPortalStatePreview);
    bindImportSource(el.settingsStateImportDrop, el.importStateFile, el.settingsStateImportText, previewPortalState, function () {
      setBackupStatus("預覽失敗；無法讀取備份檔案", true);
    });
    ...
    el.mobileSearchRiverToggle.addEventListener("click", function () {
      riversCollapsed = !riversCollapsed;
      saveRiversCollapsed();
      renderRivers();
    });
    ...
    function captureMobileScrollAnchor(preferredId, preferFollowing) {
      if (!isMobileLayout()) return null;
      var viewportBottom = window.innerHeight || document.documentElement.clientHeight || 0;
      var cards = Array.prototype.slice.call(el.gameList.querySelectorAll("[data-mobile-game-card]"));
      var visible = cards.filter(function (card) {
        var rect = card.getBoundingClientRect();
        return rect.bottom > 0 && rect.top < viewportBottom;
      });
      var preferred = String(preferredId || "");
      var preferredIndex = cards.findIndex(function (card) { return card.getAttribute("data-mobile-game-card") === preferred; });
      var following = preferredIndex >= 0 ? cards.slice(preferredIndex + 1) : [];
      var anchor = null;
      if (preferFollowing && following.length) {
        anchor = following.find(function (card) {
          var rect = card.getBoundingClientRect();
          return rect.bottom > 0 && rect.top < viewportBottom;
        }) || following[0];
      }
      if (!anchor) anchor = visible.find(function (card) { return card.getAttribute("data-mobile-game-card") !== preferred; }) || visible[0];
      if (!anchor && following.length) anchor = following[0];
      if (!anchor) anchor = cards.find(function (card) { return card.getAttribute("data-mobile-game-card") === preferred; });
      if (!anchor) return null;
      var rect = anchor.getBoundingClientRect();
      var isFollowing = following.indexOf(anchor) >= 0;
      var edge = isFollowing ? "top" : (rect.top >= 0 ? "top" : "bottom");
      return {
        id: anchor.getAttribute("data-mobile-game-card"),
        edge: edge,
        value: edge === "top" ? rect.top : rect.bottom
      };
    }

    function setMobileScrollTop(top) {
      var root = document.documentElement;
      var body = document.body;
      var previousRootBehavior = root.style.scrollBehavior;
      var previousBodyBehavior = body.style.scrollBehavior;
    ...
    function renderResultsPreservingMobileCard(id, preferFollowing) {
      var anchor = captureMobileScrollAnchor(id, preferFollowing);
      lockMobileReadObserver(700);
      renderResults();
      if (!anchor) return;
      var selector = '[data-mobile-game-card="' + String(anchor.id || "") + '"]';
      var afterCard = el.gameList.querySelector(selector);
      if (!afterCard) return;
      var afterRect = afterCard.getBoundingClientRect();
      var afterValue = anchor.edge === "top" ? afterRect.top : afterRect.bottom;
      var afterDocumentValue = afterValue + window.scrollY;
      var desiredScroll = afterDocumentValue - anchor.value;
      ...
      if (Math.abs(window.scrollY - desiredScroll) <= 0.5) return;
      lockMobileReadObserver(700);
      setMobileScrollTop(desiredScroll);
    }
    ...
    function toggleMobileEssay(id) {
      ...
      if (expandedMobileGameIds.has(id)) {
        ...
        renderResultsPreservingMobileCard(id, true);
        return;
      }
    ...
    function delegatedClick(event) {
      ...
      var similarButton = event.target.closest("[data-similar-games]");
      if (similarButton && !similarButton.disabled) {
        event.preventDefault();
        event.stopPropagation();
        createSimilarGamesRiver(similarButton.getAttribute("data-similar-games"));
        return;
      }
    ...
    function mobileRiverClick(event) {
      var openButton = event.target.closest("[data-open-river]");
    ...
    el.mobileRiverList.addEventListener("click", mobileRiverClick);
    el.mobileSearchRiverList.addEventListener("click", mobileRiverClick);
    el.backdrop.addEventListener("click", closeFilters);
    window.addEventListener("popstate", function () {
      state = readUrlState();
      pendingRiverSourceIds = null;
      pendingSearch = null;
      if (!riverInitializationPending) restoreRiverStateFromUrl();
      riverVisibleCount = loadBatchSize;
      render();
    });
    ...
    window.addEventListener("scroll", noteMobileScroll, { passive: true });
    window.addEventListener("resize", function () {
      var nextMobileLayout = isMobileLayout();
      if (nextMobileLayout === lastMobileLayout) {
        if (nextMobileLayout) {
          refreshMobileTitleMarquees();
          refreshMobileCoreMarquees();
        }
