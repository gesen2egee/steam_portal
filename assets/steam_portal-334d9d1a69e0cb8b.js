(function () {
    ...
    var REMOTE_SYNC_STATE_VERSION = 6;
    ...
    function initializePortalRivers(initialExploreGameId, renderAfter) {
      ...
      var initialActiveRiver = activeRiver();
      if (initialActiveRiver && isListRiver(initialActiveRiver) && !new URLSearchParams(window.location.search).has("sort")) {
        state.sort = normalizeListSort(initialActiveRiver.listSort);
      }
    ...
    function exportStatePayload() {
      var exportableLists = listRivers().map(function (list) { return normalizeRiver(list); }).filter(Boolean);
      var exportableRivers = rivers.filter(function (river) { return river.id !== "default"; }).map(function (river) { return normalizeRiver(river); }).filter(Boolean);
      return {
    ...
    async function loadRemoteSync(value, automatic) {
      ...
      try {
        ...
        if (selected.historyIndex >= 0) {
          ...
          setRemoteSyncStatus("已讀取同步資料；清單 " + formatNumber(selected.loaded.imported.lists.length) + " 個、固定河道 " + formatNumber(selected.loaded.imported.rivers.length) + " 條");
    ...
    function parseStateImport(rawText) {
      ...
      return {
        ...
        rivers: (Array.isArray(payload.state.rivers) ? payload.state.rivers : []).map(function (river) { return normalizeRiver(river); }).filter(function (river) { return river && river.id !== "default"; }),
    ...
    function applyImportedPortalState(imported, replaceLocal) {
      ...
      if (replaceLocal) {
        rivers = importedRivers.map(function (river) { return normalizeRiver(river); }).filter(function (river) { return river && river.id !== "default"; });
    ...
    function importPortalState(rawText) {
      ...
      setBackupStatus("已匯入並合併；清單 " + formatNumber(imported.lists.length) + " 個、固定河道 " + formatNumber(imported.rivers.length) + " 條；探索河道會重新建立");
    }
    ...
    function normalizeRiver(river) {
      ...
      return {
        ...
        listSort: listLike ? normalizeListSort(river.listSort) : "",
        filters: filters,
        ids: ids,
        position: position,
    ...
    function syncFavoriteRiver() {
      ...
      var nextFilters = existing ? normalizeRiverFilters(existing.filters) : normalizeRiverFilters({ favoritesOnly: true, sort: "river" });
    ...
    function ensureDefaultRiver() {
      ...
      if (existing && existing.ids.length) {
        ...
        var created = {
          id: "default",
          title: "探索河道",
          filters: normalizeRiverFilters(portalSettings.exploreDefaults),
          ids: ids,
          position: null,
          createdAt: 0,
          updatedAt: 0,
    ...
    function createSimilarGamesRiver(id) {
      ...
      var now = Date.now();
      var river = {
        id: "similar-" + now + "-" + Math.floor(Math.random() * 1000000),
      ...
      pendingRiverSourceIds = null;
      saveRivers();
      openRiver(river.id, true);
      showPortalToast("你建立了新搜尋河道「" + river.title + "」");
    }
    ...
    function renderRivers() {
      ...
      var defaultRiver = rivers.find(function (river) { return river.id === "default"; });
      var customRivers = rivers.filter(function (river) { return !isProtectedRiver(river.id) && river.ids.length > 0; });
      var visibleRivers;
      if (riversCollapsed) {
        visibleRivers = current && current.ids.length > 0 ? [current] : [];
      } else if (current && current.pending && current.ids.length > 0) {
      ...
      var mobileRivers = rivers.filter(function (river) { return river.ids.length > 0; }).slice().sort(riverOrder);
      if (current && current.ids.length > 0 && !mobileRivers.some(function (river) { return river.id === current.id; })) mobileRivers.unshift(current);
      var displayCurrent = current || defaultRiver;
    ...
    function updateSortOptionLabel() {
      ...
      riverOption.textContent = current && current.id === "default" ? "探索河道順序" : listActive ? "加入順序" : "河道記錄順序";
